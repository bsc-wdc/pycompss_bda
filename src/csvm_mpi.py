...
def lagrangian_fast(SVs, sl, coef, kernel):
    root_print("Length of SVs: {}".format(len(SVs)), should_print=True)
    set_sl = set(sl)
...
class Level(object):
    ...
    def scatter_data(self, X, y):
        ...
        if world_comm.rank in self.procs:
            ...
            t1 = time.time() - t0
            root_print("Total shuffling time: {}".format(t1), should_print=True)
...
def _mpi_train(base, X, y, ret="data", **kwargs):
    ...
    clf = base(**kwargs)
    if extrae:
        pyextrae.event(8000023, 2)

    clf.fit(X, y)

    if extrae:
        pyextrae.event(8000023, 0)
    ...
    if args.command == "test":
        ...
        if args.repeat == "converge":
            ...
            try:
                ...
                print("Error: repeat must be either 'converge' or a positive integer!")
                world_comm.Abort()
        ...
        root_print("Total elapsed Time: {s}s".format(s=t3), should_print=True)
